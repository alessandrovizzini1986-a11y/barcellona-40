...
ok('cartella sotto 1,2 MB', kb < 1228, `${(kb / 1024).toFixed(2)} MB`)
...
for (const [giorno, attese, crediti0] of [['ven', 14, 8], ['sab', 6, 3], ['dom', 3, 2]]) {
  ...
  ok(`${giorno}: attribuzione sotto le foto, non sotto le card nostre`, st.crediti.every(([, src, ha]) => (src || '').endsWith('.webp') ? ha : !ha), JSON.stringify(st.crediti.filter(([, src, ha]) => ((src || '').endsWith('.webp')) !== ha)))
