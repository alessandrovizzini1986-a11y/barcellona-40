// QA delle immagini delle tappe: una per tappa, e sono tutte foto vere. 14 da Wikimedia Commons e 13
...
ok('27 foto WebP: 14 da Commons più 13 private', webp.length === 27, webp.length + '')
...
ok('le tredici foto private sono dichiarate e fuori dai crediti Commons', crediti.private?.length === 13 && crediti.private.every((f) => f.id && f.file && f.tappa && /concessione/i.test(f.nota) && !crediti.foto.some((c) => c.id === f.id)), String(crediti.private?.length))
...
ok('CREDITS.md dice che le card disegnate non ci sono più', /Card stilizzate \(originali del progetto\)/.test(cred) && /Per Barcellona non ce ne sono più/.test(cred))
...
for (const [giorno, attese, crediti0] of [['ven', 16, 16], ['sab', 9, 9], ['dom', 3, 3]]) {
