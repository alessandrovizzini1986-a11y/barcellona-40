...
ok('Bunkers: il tramonto astronomico e la cresta stanno nei dettagli', stops.d2.details.some((d) => d.includes('19:07') && d.includes('18:58')), stops.d2.details.join(' | '))
ok('Bunkers: le 18:58 portano la fonte del calcolo', stops.d2.details.some((d) => /SRTM 30 m/.test(d) && /azimut 256/.test(d) && /1,14/.test(d)))
...
ok('Bunkers: niente più la stima delle 18:50', !JSON.stringify(stops.d2).includes('18:50'))
ok('Bunkers: il "perché" parla della città che si accende', /si accende/.test(stops.d2.why) && stops.d2.why.includes('18:58'), stops.d2.why)
ok('Bunkers: nel blocco stimato restano solo taxi e discesa', stops.d2.detailsStimati?.length === 2 && stops.d2.detailsStimati.every((d) => /taxi|piedi/i.test(d) && !stops.d2.details.includes(d)))
ok('Bunkers: la cresta non è più una stima', !stops.d2.detailsStimati.some((d) => /Collserola/.test(d)))
ok('Bunkers: nessun consiglio sul taxi fra i dettagli verificati', !stops.d2.details.some((d) => /prenota il taxi/i.test(d)))
ok('domenica: si scende alle 19:30, non alle 19:00', passo('d1').ora === '19:30' && /18:58/.test(passo('d1').dettaglio || ''), `${passo('d1').ora} · ${passo('d1').dettaglio}`)
