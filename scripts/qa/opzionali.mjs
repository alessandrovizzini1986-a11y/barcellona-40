...
for (const id of ['f99', 's99']) {
  ...
  ok(`${id}: l'avviso sul documento è un dettaglio, in maiuscolo`, s.details.some((d) => d.startsWith('SERVE IL DOCUMENTO ORIGINALE') && /Niente fotocopie/.test(d)))
  ok(`${id}: aperto 24 ore, guardaroba, minimi, ristorante`, ['Aperto 24 ore', 'guardaroba', 'Minimi alti', 'ristorante interno'].every((t) => s.details.some((d) => d.includes(t))))
...
for (const [giorno, id] of [['ven', 'f99'], ['sab', 's99']]) {
  ...
  ok(`${giorno}: l'avviso sul documento si legge`, /SERVE IL DOCUMENTO ORIGINALE/.test(await card.innerHTML()))
