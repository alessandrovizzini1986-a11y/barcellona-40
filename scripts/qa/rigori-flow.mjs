...
await step('chiTira', async () => { await waitFlow('chiTira'); await p.waitForTimeout(400); await shot('f9-02-chi-tira'); await p.click('.rg-card[data-value="monne"]'); await p.waitForFunction(() => window.__rigori.flow() === 'giocatore'); await p.click('[data-value="vai"]') })
