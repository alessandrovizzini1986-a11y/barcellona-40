...
export function createPassAndPlay({ names = ['Ale', 'Monne'], rounds = 3 } = {}) {
  ...
  return makeMode({
    id: 'passAndPlay', title: 'Pass-and-play', players, rounds, round: 1, i: 0, avanza: false, keeperZone: null, phase: 'pick', // pick → shoot
    ...
    nextTurn(ctx) {
      ...
      // la rotazione avviene qui, a replay finito: durante esito e replay l'HUD resta sul turno del tiro
      if (this.avanza) { this.avanza = false; this.i++; if (this.i >= players.length) { this.i = 0; this.round++ }; if (this.round > this.rounds) { this.finished = true; ctx.hud('Classifica di serata'); return } }
      this.hud(ctx)
      if (this.phase === 'pick') {
        ctx.role('idle')
    ...
    onResult(res, ctx) {
      ...
      const cosa = res.result === 'goal' ? `Gol di ${this.shooter.name}` : res.result === 'save' ? `Parata di ${this.keeperP.name}` : res.result === 'miss' ? `${this.shooter.name} fuori` : res.result === 'crossbar' ? 'Traversa' : 'Palo'
      ctx.hud(`Giro ${this.round}/${this.rounds} · ${cosa} · ${this.shooter.name} ${this.shooter.goals} · ${this.keeperP.name} ${this.keeperP.saves}`)
      this.avanza = true
    },
